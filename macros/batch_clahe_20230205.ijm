...
save_dir = data_dir+File.separator+"clahe_processed";
...
for(i=0; i<fileList.length; i++){
	if(endsWith(fileList[i], file_extension)){
		...
		for(j = 1; j <= slices; j++){
			...
			setSlice(j);
			run("Enhance Local Contrast (CLAHE)", "blocksize=127 histogram=512 maximum=3 mask=*None*");
		};
		
		saveAs("Tiff", save_dir+File.separator+title+"_clahe.tif");
		run("Close All");
	}
}
